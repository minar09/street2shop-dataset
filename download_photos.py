            ...
            if r.status_code == 200:
                ...
                if image_type is not None:
                    with open(os.path.join(images_dir, item_id + '.' + image_type), 'wb') as f:
                        f.write(r.content)
                        f.close()
                    logging.info('Success: %s\t%s\tstatus:%d' % (item_id, url, r.status_code))
                else:
                    logging.error('Error: %s\t%s\tunknown_type' % (item_id, url))
            else:
                logging.error('Error: %s\t%s\tstatus:%d' % (item_id, url, r.status_code))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--urls', dest='urls', type=str, default='photos/photos.txt', help='urls')
    # parser.add_argument('--image_dir', dest='images_dir', type=str, default='images', help='image directory')
    parser.add_argument('--image_dir', dest='images_dir', type=str, default='D:\Datasets\Street2Shop\images', help='image directory')
